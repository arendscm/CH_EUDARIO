...
SC_registry <- read_excel("data/external/SC_registry.xlsx",sheet = "SC-IDs")

###include only new samples = those with Patient.ID = NA
df -> df.orig
df.orig %>% 
  filter(is.na(Patient.ID)) %>% 
  dplyr::select(-c1d1_cf,-c1d1_wb,-c7d1_cf,-ue_cf,-eot_cf,-eot_wb,-sum_wb,-sum_cf,-Patient.ID,-Visite,-Material) %>%
  left_join(.,SC_registry, by="Sample_orig") %>%
  mutate(mutID_pat = paste(Patient.ID,position,sep="_"))%>%
  group_by(mutID_pat) %>%
  mutate(n.mut = n()) %>%
  data.frame -> df
...
df %>%
  ...
  dplyr::select(mutID) %>% 
    mutate(mutID = as.character(mutID)) -> mutID.func
...
write.xlsx(df.filtered.newsamples,filename,sheetName = "filtered_results",append=TRUE)
