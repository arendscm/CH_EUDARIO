...
full_join(df.cf,df.cf_wb,by="cfID") %>% 
  ...
  filter(tag.x %in% c("true","cf-only")|tag.y %in% tag.y %in% c("true","cf-only"))%>%
  mutate(compartment = ifelse(TVAF.x > TVAF.y*3,"cf","wb"))%>% ##classify according to compartment (cf or wb)
  mutate(chr = Chr.x,
