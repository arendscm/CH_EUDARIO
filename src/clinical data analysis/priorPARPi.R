...
df$PriorPARPi %>% table %>% data.frame -> n.parp
names(n.parp) <- c("PriorPARPi","n.parp")

genes=c("DNMT3A","TET2","ASXL1","PPM1D","TP53","CHEK2","ATM")

df.filtered.c1d1 %>% left_join(.,df %>% group_by(PriorPARPi) %>% mutate(n.parp = n()) %>% data.frame %>% dplyr::select(Patient.ID, PriorPARPi,n.parp))%>%
  filter(tag == "true") %>%
  filter(TVAF >= 0.01) %>%
...
df$No_Platinum_lines_binom %>% table %>% data.frame -> n.plat
names(n.plat) <- c("No_Platinum_lines_binom","n.plat")

genes=c("DNMT3A","TET2","ASXL1","PPM1D","TP53","CHEK2","ATM")

df.filtered.c1d1 %>% left_join(.,df  %>% dplyr::select(Patient.ID, No_Platinum_lines_binom))%>%
  filter(tag == "true") %>%
  filter(TVAF >= 0.01) %>%
  filter(Gene %in% genes)%>%
...
log.reg <- glm(CH ~ Age_TreatmentStartEUDARIO + PriorPARPi + Number_PreviousPlatinumLines, family="binomial",data=df)
summary(log.reg)
